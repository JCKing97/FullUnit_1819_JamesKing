...
12 unique experiments were conducted with this population. I would have liked to duplicate the experiments, however again due to time constraints (the experiments take hours to run with long generation lengths) and the overwhelmingly similar results between all experiments I did not deem this necessary nor beneficial. These experiments can be divided into sets of 3 experiments.\par 
...
My plan had originally been to review the outcomes of my robustness experiments and pick strategies which has been successful for the next set of experiments. These were to be similar to Axelrod and Hamilton's~\cite{evolution_of_cooperation} initial viability experiments, in which individuals using a cooperative strategy were placed within a society made up of predominantly non-cooperative players, to see if they could gain a foothold in these societies.\par 
However, my robustness experiments revealed no cooperative strategies capable in surviving in a varied environment. Due to this, I decided against running any initial viability experiments as the results are overwhelmingly likely to be unsuccessful in finding any initially viable strategies, unless the conditions were drastically changed.

\section{Stability}
Axelrod and Hamilton~\cite{evolution_of_cooperation} also ran another set of experiments to test what strategies could prevent defection from taking hold in societies that are predominantly cooperative. Again I had planned to take successful strategies from my robustness experiments and use them in this. However, of course I did not find any successful strategies.\par 
This type of experiment is different to initial viability though, and gives cooperation more of a chance. As such I designed and ran experiments similar to Axelrod and Hamilton's stability experiments. I selected 9 strategies with a variety of strategies and trust models (listed in the appendix in section~\ref{appendix:stab_strats}) that I believe could be successful in upholding cooperation against a group of 6 defectors.\par 
In this group of defectors are 2 for each of the non-donor strategies: Lazy, Promote Self and Spread Negative. For each of the 10 selected strategies I ran 6 experiments. This can be again split into two sets of 3 experiments with the first set of generation length 100 and the second of generation length 300. All experiments ran for 10 generations and again each set of 3 experiments had one experiment with 5 onlookers per interaction, one with 10 and another with 20.
...
\begin{itemize}
	...
	\item Image Scoring Discriminator, Promote Self, Trusting, K=0
	\item Image Scoring Discriminator, Spread Accurate Positive, Distrusting, K=0, Personal Grievance
	\item Veritability Discerner, Spread Negative Untrusted, Strong Reactor, K=-5
	\item Veritability Discerner, Spread Positive Trusted, Forgiving Reactor, K=0
	\item Veritability Discerner, Promote Self, Balanced Reactor, K=0
\end{itemize}
