...
\textcolor{red}{Explain subset I have taken and why I have chosen it (middle, good and not so good) (Ratio for strategies and eighth) (Duplicate experiments) (R, IV, S)\\ Because the environment is not visible and there is so much random chance in the system unlike other game-theoretic models, wildly different results are produced each time. Affirming that past game-theoretic analysis of systems isn't adequate.}
\section{Introduction}


\section{Robustness}
The experiments I have run are similar to Axelrod and Hamilton's~\cite{evolution_of_cooperation} experiments on the robustness of strategies in the system. There are 199 strategies available to use in my system, if I included one of each of these in the experiments it would overload the system. As such, I selected a population using a subset of these strategies to for the experiments that remains the same throughout the robustness experiments (listed in appendix section~\ref{appendix:population}).\par 
9 unique experiments were conducted with this population and each one was duplicated - I would have liked to triplicate the experiments, however due to time pressure this was not possible. The first 3 experiments ran for 10 generations with a generation length of 60. These 3 experiments were different due to the number of onlookers selected for each on - 5, 10 and 20 respectively.\par 
The next 3 experiments were reduced to a generation number of 5 due to the time it took to run them with 10 generations. These experiments were conducted with a generation length of 120. Again the number of onlookers was changed between each with the values 5, 10 and 20 respectively. The final set of 3 experiments were again conducted with a generation number of 5 but this time with a generation length of 200, and the same selected number of onlookers for each as the other sets of 3 experiments.\par 
These experiments fulfilled a number of purposes. One of these was to review my framework for how well it supports the evolution of cooperation, another was to review which strategies were most consistently successful and additionally I evaluated the effect gossip and trust models to interpret this gossip have on the games.

\subsubsection{Remark: Consistency of Results}
Originally I ran a number of experiments at generation length 20. Players in these games reached a maximum total fitness of 35. These low fitness scores did not provide the reproduction algorithm with much chance of reflecting how successful a strategy can be with the amount they were reproduced into the next generation. This led to a remarkably low level of reproducibility of results. Even duplicate experiments returned wildly dissimilar results.\par 
I diregarded these results for the following reasons. If a particular strategy got lucky with an early round of reproduction, it is likely that they would dominate the society later on as agents using that strategy are more likely to be randomly chosen in the reproduction algorithm before being stochastically accepted or not.\par 
Another factor in this is the way interactions are selected. The donor-recipient pairs are selected at random from the population. If an agent or multiple agents of the same strategy were lucky enough to be selected a number of times as a recipient and receive cooperation, they are more likely to be reproduced out of pure luck over a good strategy.

\subsection{Evaluation: Generation Length}


\subsection{Evaluation: Evolution of Cooperation and The Framework}


\subsection{Evaluation: Strategies}
\textcolor{red}{Defection and cooperation able to exist alongside each other}
...
\section{Robustness Experiments Population}
