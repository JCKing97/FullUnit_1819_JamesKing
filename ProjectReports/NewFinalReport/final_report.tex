...
\textcolor{red}{Learning flask + Prolog web, Proof of concept applications, Talk about web dev practices, Architecture, Amaas, Web Application (+environment), Database (SQLAlchemy + SQLite + MySQL), Deployment}
...
\vspace{-30pt}
\end{wrapfigure}
In addition to this I have created a feature that allows users to create accounts, login and associates games of my theoretical framework to their account. These games can be given a label so they can keep track of the experiments the have run (illustrated in figure~\ref{fig:user_experiments}. User data is recorded in the database and passwords are hashed to keep them secret. 

\subsection{Framework}
\textcolor{red}{Agents based, Strategies, environment etc.}
\subsubsection{Environment and Agent's Bodies}
The environment and agent's bodies from the theoretical framework described in section~\ref{sec:theo} has been implemented within the nature engine. 
...
\textcolor{red}{Unit testing (PLUnit + unittest), Web App Testing, API Testing, UML design, Design Patterns (Observer of player state), GUI Design, Scrum (Sprints, Task Backlog), Documentation (sphinx and restructured text, pldoc) and commenting, API Documentation, Agent Oriented Programming, Git (VCS), Tools (Pycharm, UMLet, Linting, Syntax highlighting, packager)}
\subsubsection{Process}

\subsubsection{Design}

\subsubsection{Testing}
The testing of my system can be broken down into five parts: AMAAS bottom up integration testing, environment bottom up integration testing, application programming interface (API) testing, web application testing and some extra early unit testing on other parts of the web application. The bulk of this testing comes from the first two, which were interleaved with the development of the system. The early unit testing was also interleaved with development. However API and web application testing was carried out after development.\par 
...
\textcolor{red}{Things on to do list + in conclusions}
