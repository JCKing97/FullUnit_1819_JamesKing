...
class GenerationTest(unittest.TestCase):
    ...
    def test_get_onlookers(self):
        try:
            ...
            onlookers = generation._generate_onlookers({'type': 'action', 'value': 'defect', 'donor': donor_id,
                                                        'recipient': recipient_id})
            self.assertEqual(self.num_of_onlookers+2, len(onlookers))
            self.assertTrue(generation._id_player_map[donor_id] in onlookers)
            self.assertTrue(generation._id_player_map[recipient_id] in onlookers)
        except GenerationCreationException:
            self.fail("Should not have failed to create generation")
